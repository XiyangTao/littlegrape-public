...
import type { SentenceShuffleQuestion } from '@/api/modules/exercise';

interface Props {
  question: SentenceShuffleQuestion;
  isAnswered: boolean;
...
export default function SentenceShuffleView({ question, isAnswered, onAnswer, onSubmitReady, styles, theme, submitRef }: Props) {
  const { t } = useI18n();
  ...
  return (
    <View>
      <Text style={styles.questionType}>{t('exercise.sentenceShuffle.prompt')}</Text>
      ...
      <View style={[styles.answerSlot, arranged.length > 0 && styles.answerSlotFilled]}>
        {arranged.length === 0 ? (
          <Text style={{ color: theme.colors.text.disabled }}>{t('exercise.sentenceShuffle.placeholder')}</Text>
        ) : (
          arranged.map((word, i) => (
            <TouchableOpacity
              key={`${word}_${i}`}
