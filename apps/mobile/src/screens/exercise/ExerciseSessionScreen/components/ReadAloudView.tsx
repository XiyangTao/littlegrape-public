...
import type { ReadAloudQuestion } from '@/api/modules/exercise';
...
export default function ReadAloudView({ question, isAnswered, onAnswer, styles, theme }: Props) {
  const { t } = useI18n();
  ...
  return (
    <View>
      <Text style={styles.questionType}>{t('exercise.readAloud.prompt')}</Text>

      <View style={styles.promptCard}>
        <Text style={[styles.promptText, { fontSize: 20 }]}>{question.sentence}</Text>
      ...
      {!isAnswered && (
        <>
          ...
          <Text style={{ textAlign: 'center', color: theme.colors.text.tertiary, fontSize: 13 }}>
            {pronunciation.isRecording ? t('exercise.readAloud.recording') :
             pronunciation.isAssessing ? t('exercise.flashcard.assessing') : t('exercise.readAloud.tapToRead')}
          </Text>
        </>
      )}
