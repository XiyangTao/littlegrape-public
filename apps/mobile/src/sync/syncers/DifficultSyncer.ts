 ...
 *
 * 策略：local-first（本地优先）
 * - 初始化时：从服务端增量拉取生词（游标分页 + afterServerTime）
 */

import { apiClient } from '@/api';
import { upsertDifficultFromServer } from '@/db/WordDB';
import { getSyncServerTime, saveSyncServerTime } from '@/db/SyncMetadataDB';
import type { Syncer } from '../types';

const SYNCER_NAME = 'difficult';

export const DifficultSyncer: Syncer = {
  name: SYNCER_NAME,
  priority: 3,
  ...
  initSync: async (userId: string) => {
    if (__DEV__) console.log(`[Sync:${SYNCER_NAME}] 从服务端拉取生词本数据...`);
    try {
      // 获取上次同步时间（增量同步）
      ...
      // 游标分页拉取所有数据
      while (true) {
        const response = await apiClient.pullDifficultWords({
        ...
        const { words, nextCursor, serverTime } = response.data;

        if (words.length > 0) {
          const insertedCount = await upsertDifficultFromServer(userId, words);
          totalImported += insertedCount;
          if (__DEV__) console.log(`[Sync:${SYNCER_NAME}] 本批次: 服务端返回 ${words.length} 条, 实际插入 ${insertedCount} 条`);
        }
